    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      const [insertResult] = await db.query(
        `INSERT INTO company_images (company_id, url, public_id, is_cover, sort_order)
         VALUES (?, ?, ?, ?, ?)`,
        [company_id, result.secure_url, result.public_id, isCover, existing[0].total + i]
      );
      ...
      console.log(`[company-image] uploaded company=${company_id} public_id=${result.public_id} cover=${isCover}`);
      await redis.del(redis.KEYS.company(company_id), redis.KEYS.allCompany);
    }
...
const setCompanyCover = async (req, res) => {
  try {
    ...
    if (!rows.length)
      return res.status(404).json({ message: 'Image not found for this company' });

    // Unset all covers for this turf then set the new one
    await db.query(
      `UPDATE company_images SET is_cover = 0 WHERE company_id = ?`, [company_id]
    );
    await db.query(
      `UPDATE company_images SET is_cover = 1 WHERE id = ?`, [image_id]
    );

    await redis.del(redis.KEYS.company(company_id), redis.KEYS.allCompany);
    console.log(`[company-image] cover set company=${company_id} image=${image_id}`);
...
const deleteCompanyImage = async (req, res) => {
  try {
    ...
    if (image.is_cover) {
      ...
      if (remaining.length) {
        await db.query(
          `UPDATE company_images SET is_cover = 1 WHERE id = ?`, [remaining[0].id]
        );
      }
    }

    console.log(`[company-image] deleted company=${company_id} image=${image_id} public_id=${image.public_id}`);
...
const deleteAllCompanyImages = async (company_id) => {
  try {
    const [rows] = await db.query(
      `SELECT public_id FROM company_images WHERE company_id = ?`, [company_id]
    );
    ...
    console.log(`[company-image] deleted all ${rows.length} images for company=${company_id}`);
  } catch (err) {
